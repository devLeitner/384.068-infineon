...
#IMAI_DATA_OUT_SYMBOLS = ["unlabelled", "a", "b", "c"]
IMAI_DATA_OUT_SYMBOLS = ["unlabelled", "air", "plastic", "plastic_out", "a", "b"]
...
print(model.get_model_output_dim())
...
while True:
    ...
    for i in range(num_samples):
        ...
        #if i == 0:
        #    print("Raw:", raw_sample, "Norm:", normalized_sample)
        # Pass the sample to the model
        
        enq_status = model.enqueue([normalized_sample])
        if enq_status != 0:
            print("Could not enqueue", enq_status)
